({
    saveProductsToWTP: function(component, WTPId, sProductList, deleteProductList, selectedSharesList, wtpId){
        debugger;
        ...
        var action = component.get("c.linkProductsToWTP");

        console.log('SLI <<>> ', selectedSharesList);
        console.log('SLI <<>> W ', wtpId);

        action.setParams({
            "insertProductList": insertProductList,
            "deleteProductList": deleteProductList,
            "selectedShares": selectedSharesList,
            "wtpId": wtpId
        });
        action.setCallback(this, function(response){
            var state = response.getState();            
            if (state === 'SUCCESS'){
                this.navigateToObject(WTPId);
            } 
        });
