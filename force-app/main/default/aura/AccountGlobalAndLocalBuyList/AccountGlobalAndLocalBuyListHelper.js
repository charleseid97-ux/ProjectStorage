({
    ...
    loadData : function(component) {
        //load action
        var action = component.get("c.loadGlobalAndLocalBuyList");
        ...
        //set callback action
            ...
            if(component.isValid() && response.getState() === "SUCCESS") {
                ...
                component.set("v.noGlobalList", noGBL);
    ...
    loadFormaterData : function(component) {
        ...
        for(var i = 0; i < maxItem; i ++) {
            ...
            var currentObject = {editAccess :data[i].editAccess , deleteAccess : data[i].deleteAccess, Id : data[i].Id, Status : data[i].Status__c};
            currentObject.data = [];
    ...
    editRecord : function(component, recordId) {
        if( component.get("v.customViewManagement")) {
            this.loadCustomView(component, "EDIT", recordId);
            return;
        }
    ...
    loadCustomEdit : function(component,mode, objectId) {
        debugger;
        $A.createComponent(
            "c:EditIndependantBuyList",
            {
                parentObjectId: component.get('v.recordId'),
                sObjectType: component.get("v.sObjectType"),
                mode : 'EDIT',
            ...
            function(viewComponent, status, errorMessage){
		        debugger;
                if (status === "SUCCESS") {
                    component.set("v.independantEditComponent", viewComponent);
                    component.set('v.preview', true);
                }
                else if (status === "INCOMPLETE") {
                    console.log("No response from server or client is offline.")
    ...
    DeleteIndependent : function(cmp, recordId) {
