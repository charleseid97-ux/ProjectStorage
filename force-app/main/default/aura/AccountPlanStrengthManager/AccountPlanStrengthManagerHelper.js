({
    loadStrength: function(component, recId) {
        var action = component.get("c.loadPlanStrength");
	...
	saveStrength: function(component) {
        var action = component.get("c.saveStrength");
        var SandW = component.get("v.strengthsAndWeaknesses");
        var parentRecordIdVal = SandW.Company__c;
        
        action.setParams( {
            parentRecordId : parentRecordIdVal, 
            currentObjectId : component.get("v.currentObjectId"),
            strength : SandW.Strength__c,
            strengthType : SandW.Strength_Type__c
        });
        action.setCallback(this, function(response) {
            if(component.isValid() && response.getState() === "SUCCESS") {
